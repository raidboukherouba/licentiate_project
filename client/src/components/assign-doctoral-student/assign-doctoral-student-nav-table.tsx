...
import { exportAssignDoctoralStudents } from "@/services/assignDoctoralStudentService";
...
interface AssignDoctoralStudentNavTableProps {
  searchQuery: string;
...
export default function AssignDoctoralStudentNavTable({
  searchQuery,
  setSearchQuery,
  itemsPerPage,
  setItemsPerPage,
  ...
  visibleColumns,
  setVisibleColumns,
}: AssignDoctoralStudentNavTableProps) {
  ...
  const handleExport = async () => {
    try {
      const blob = await exportAssignDoctoralStudents(searchQuery, selectedLab || undefined);
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "doctoral_student_assignments.xlsx";
  ...
  // Navigate to the Add AssignDoctoralStudent page
  const navigateToAddAssignDoctoralStudentPage = () => {
    navigate('/equipment-management/assign-doctoral-student/add-assignment');
  };
      ...
      <div className="w-full">
        <div className="flex flex-col gap-2 md:flex-row md:justify-between md:items-center mb-4">
          ...
          <div className="relative w-full md:w-70">
            ...
            <Input
              type="text"
              placeholder={t("assignDoctoralStudent.search_placeholder")}
              value={searchQuery}
              onChange={(e) => {
          ...
          <div className="flex flex-col gap-2 md:flex-row md:items-center">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-40">
                  {Object.keys(visibleColumns).map((column) => (
                    ...
                    >
                      {t(`assignDoctoralStudent.${column}`)}
                    </DropdownMenuCheckboxItem>
                  ))}
                </DropdownMenuContent>
              </DropdownMenu>
            ...
            {/* Add AssignDoctoralStudent Button */}
            <Button
              variant="outline"
              size="sm"
              onClick={navigateToAddAssignDoctoralStudentPage}
