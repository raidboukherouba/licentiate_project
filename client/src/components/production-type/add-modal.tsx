...
interface AddModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (name: string) => void; // Accept the production type name
}
...
export function AddModal({ isOpen, onClose, onAdd }: AddModalProps) {
  ...
  const form = useForm<z.infer<typeof productionTypeSchema>>({
    ...
    defaultValues: {
      type_name: "", // Initialize with an empty string
    },
  });
  ...
  useEffect(() => {
    if (isOpen) {
      form.reset({ type_name: "" });
    }
  }, [isOpen, form]);

  // Handle form submission
  const handleAdd = (values: z.infer<typeof productionTypeSchema>) => {
    onAdd(values.type_name); // Pass the validated production type name
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t("productionType.add_title")}</DialogTitle>
        </DialogHeader>
        {/* <DialogDescription>
          {t("productionType.add_description")}
        </DialogDescription> */}
        <Form {...form}>
