...
import { assignDoctoralStudentToPublication, removeDoctoralStudentFromPublication } from '@/services/publishDoctoralStudentPubService';
...
export default function DoctoralStudentPublicationsPub() {
  const { doi } = useParams();
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setAssignedStudents(publicationData.publication.DoctoralStudents || []);
      } catch (error) {
  ...
  const handleAssign = async (reg_num: number) => {
    try {
      const result = await assignDoctoralStudentToPublication(reg_num, doi as string);
      ...
      toast.success(t('publication.studentAssigned'));
    } catch (error) {
  ...
  const handleRemove = async (reg_num: number) => {
    try {
      const result = await removeDoctoralStudentFromPublication(reg_num, doi as string);
      ...
      toast.success(t('publication.studentRemoved'));
    } catch (error) {
  ...
  return (
    <div className="container mx-auto p-2">
      <div className='flex justify-between gap-2 bg-sidebar p-2 mb-4'>
        <h1>
          <span className='font-semibold'>{t('publication.manageStudents')}:</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <h2 className="text-lg font-semibold mb-3 pl-2">{t('publication.availableStudents')}</h2>
        ...
        <div>
          <h2 className="text-lg font-semibold mb-3 pl-2">{t('publication.assignedStudents')}</h2>
          <div className="space-y-2">
            {assignedStudents.length > 0 ? (
              ...
              <p className="text-muted-foreground">{t('publication.noStudentsAssigned')}</p>
            )}
          </div>
        </div>
      </div>
