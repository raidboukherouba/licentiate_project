...
interface AddModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (name: string) => void; // Accept the speciality name
}
...
export function AddModal({ isOpen, onClose, onAdd }: AddModalProps) {
  ...
  const form = useForm<z.infer<typeof specialitySchema>>({
    ...
    defaultValues: {
      spec_name: "", // Initialize with an empty string
    },
  });
  ...
  useEffect(() => {
    if (isOpen) {
      form.reset({ spec_name: "" });
    }
  }, [isOpen, form]);

  const handleAdd = (values: z.infer<typeof specialitySchema>) => {
    onAdd(values.spec_name); // Pass the validated speciality name
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t("speciality.add_title")}</DialogTitle>
        </DialogHeader>
        {/* <DialogDescription>
          {t("speciality.add_description")}
        </DialogDescription> */}
        <Form {...form}>
