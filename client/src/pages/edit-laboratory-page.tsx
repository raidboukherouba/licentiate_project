...
import { fetchLaboratoryById, updateLaboratory } from '../services/laboratoryService';
...
export default function EditLaboratoryPage() {
      ...
      try {
        ...
        if (labCode) {
          ...
          form.reset({
            ...
            lab_abbr: laboratory.lab_abbr || "",
            lab_desc: laboratory.lab_desc || "",
            lab_address: laboratory.lab_address || "",
            lab_phone: laboratory.lab_phone || "",
            faculty_id: laboratory.faculty_id,
            domain_id: laboratory.domain_id,
            dept_id: laboratory.dept_id,
          });
        }
        }
      } catch (error: any) {
  ...
  const handleEdit = async (values: z.infer<typeof laboratorySchema>) => {
    if (!labCode) return;

    try {
      const response = await updateLaboratory(
        Number(labCode),
        values.lab_name,
        values.faculty_id,
        values.domain_id,
  ...
  return (
    <div className="container mx-auto px-4">
      <h1 className="text-2xl font-bold mb-4">{t("laboratory.edit_title")}</h1>
      <Form {...form}>
