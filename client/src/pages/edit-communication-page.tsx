...
import { fetchCommunicationById, updateCommunication } from '../services/communicationService';
...
export default function EditCommunicationPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (idComm) {
          const communicationResponse = await fetchCommunicationById(Number(idComm));
          if ('communication' in communicationResponse) {
            const communication = communicationResponse.communication;
            
            form.reset({
              title_comm: communication.title_comm,
              event_title: communication.event_title,
              year_comm: communication.year_comm,
              url_comm: communication.url_comm || "",
  ...
  const handleEdit = async (values: z.infer<typeof communicationSchema>) => {
    if (!idComm) return;

    try {
      const response = await updateCommunication(Number(idComm), {
  ...
  return (
    <div className="container mx-auto px-4">
      <h1 className="text-2xl font-bold mb-4">{t("communication.edit_title")}</h1>
      <Form {...form}>
