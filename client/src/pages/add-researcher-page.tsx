...
export default function AddResearcherPage() {
  ...
  const handleAdd = async (values: z.infer<typeof researcherSchema>) => {
    try {
      const response = await createResearcher(
        values.res_fname,
        values.res_lname,
        values.res_fname_ar,
        values.res_lname_ar,
  ...
  return (
    <div className="container mx-auto px-4">
      <h1 className="text-2xl font-bold mb-4">{t("researcher.add_title")}</h1>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleAdd)} className="space-y-6">
          ...
          <div className="flex justify-end space-x-4">
            <Button size="sm" variant="outline" onClick={() => navigate('/personnel-management/researchers')}>
              {t("global.cancel")}
            </Button>
